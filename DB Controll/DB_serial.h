...
bool db_cmd(uint8_t cmd, uint8_t lenIn, uint8_t* parameterIn, uint8_t* parameterOut) {
	...
	uint8_t* bufferIn;
	static int cnt = 0;
	uint8_t buffer[DB_MAXLENGTH];
	uint8_t buffer2[DB_MAXLENGTH];
	bufferIn = (uint8_t*)malloc((2 + lenIn));
	bufferIn[0] = cmd;
	bufferIn[1] = (uint8_t)lenIn;
	if (parameterIn != NULL) {
		for (uint8_t i = 2; i != lenIn + 2; i++) {
			bufferIn[i] = parameterIn[i - 2];
		}
	}
	write_port(db_port, bufferIn, (lenIn + 2));
	if (db_monitor) {
		printf("\nSET %02X %02X :", bufferIn[0], bufferIn[1]);
		for (uint8_t i = 0; i != bufferIn[1]; i++)
			printf(" %02X", bufferIn[2 + i]);
		printf("\n");
	}
	...
	case DB_MC:
		...
		break;
	case DB_MC_D:
		if (buffer2[0] != DB_MC_D || buffer2[1] != ((12 * 2) + 2)) {
			return 0;
		}
		break;
	case DB_EEPROM:
